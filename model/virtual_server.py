...
async def process(request):
    ...
    if request.method == 'POST':
        ...
        video_file = await reader.next()

        # Check if the field is a file and has a filename
        ...
        webcam = None
        if len(sys.argv) == 2:
            webcam = cv2.VideoCapture("video_capture.mp4")
        else:
            webcam = cv2.VideoCapture("video_capture.mp4")
        ...
        # hr_values = [60, 65, 70, 75, 80, 85, 90, 95]
            ...
            final_activity = "Go explore nature."
            if final_activity == "normal":
                ...
                # Add emotions and final_activity to the response
            response['emotions'] = emotions
            response['final_activity'] = final_activity
...
app.router.add_post("/api", process)
...
web.run_app(app, host='0.0.0.0', port='1010')