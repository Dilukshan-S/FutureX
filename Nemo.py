...
pulse_bpm = round(pulse * 60 * 1.21, 2)


# Calculate percentage of different emotions
angry_disgust_count = emotions['Angry'] + emotions['Disgust']
emotion_counts = {'Stressed': angry_disgust_count, 'Anxiety': emotions['Anxiety'], 'Sad': emotions['Sad']}
...
filename = f"emotion_data_{now.strftime('%Y-%m-%d_%H-%M-%S')}.txt"
with open(filename, 'w') as f:
    f.write("Emotion percentages:\n")
    for label, count in emotion_counts.items():
        percentage = count / total_frames
        f.write(f"{label}: {percentage:.2f}\n")
